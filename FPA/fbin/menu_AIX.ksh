  ...
  do
    ...
    if [ "${it}" = "" ];then            # no selection was made
      ...
      if [ ! -w ${BATCH_TMP}/menu_parameters$$.txt ]; then 
        echo "INTERNAL ERROR in temporary parameter file...  Notify FPA Administrator. \nPress [ENTER] to continue...\v"
        read ll
...
ca=`echo '\001'`  ####  JPT 20051115  Added the ca variable to contain the control-a character for use in sed commands.
