...
# 05/03/2007   J Thiessen       Added a concise log file based on BATCH_DAY 
# 07/27/2007   R Crawford       Addition of get option.
# 11/07/2007   J Thiessen       -
#*******************************************************************************
...
analyze_session()
{
  ...
  ####  
  ####  Create the Daily FTP summary log    ##  JT 20070503
  ####  
  if [ -a ${BATCH_LOGS}/BatchDay.[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9] ]; then
    my_batchday_file=`ls -1r ${BATCH_LOGS}/BatchDay.[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]  | head -1`
  else
    my_batchday_file=${BATCH_LOGS}/BatchDay.`date +%Y%m%d`
  fi
  
  my_batchday=`basename ${my_batchday_file}`     ## BATCHDAY at the time this function is called, not the time of the actual FTP. 
  my_ftp_summary_log="${BATCH_LOGS}/ftplog.${ftp_dest}.${my_batchday}"    
  
  ####  
  ####  Create the log file to be analyzed, translating each leading space
  ####  
  echo "FTP ANALYZING SESSION  -- PID $$ `date` "  >> ${batch_log}.debug 
  echo "## ftp_tmp_cmd_file = ${ftp_tmp_cmd_file} "  >> ${batch_log}.debug 
  echo "ANALYZING SESSION"
  analyzelog=${BATCH_TMP}/`basename ${batch_log}`
  sed 's/^ /-- /' ${batch_log} > ${analyzelog}
  echo "## analyzelog = ${analyzelog} "  >> ${batch_log}.debug 
  echo "######  FTP ANALYZING SESSION for `basename ${batch_log}` with PID $$ on `date` "  >> ${my_ftp_summary_log}   ##  JT 20070503
  
  ####  
  ####  Open and properly position the descriptor to read the FTP commands file for this session.
  ####  
  ...
  do
    if read -u5 code line ;then
      echo "STATUS=$?   CODE--->${code}<---  LINE--->${line}<---"  >> ${batch_log}.debug 
      loglineno=`expr ${loglineno} + 1`
      if [ "${code}" = "BEGINNING_FTP_CONNECTION:" ]; then
        leaveloop="yes"
      fi
    else
      echo "STATUS=$?   CANNOT READ u5 -- MUST BE EOF"  >> ${batch_log}.debug 
      leaveloop="yes"
      leavefunction="yes"
      detected_error=1
      err_count=`expr ${err_count} + 1`
  ...
  do
    ...
    if read -u4 t_cmd t_cmd_args ;then
      my_logfile_trailer="${my_retcod:-First} ${my_retstr:-Loop}"       ## JPT 20060220 
              echo "READ u4 in analyze_session: THE COMMAND is    t_cmd = --->${t_cmd}<---" >> ${batch_log}.debug 
              echo "READ u4 in analyze_session: THE ARGS are t_cmd_args = --->${t_cmd_args}<---" >> ${batch_log}.debug 
      if read -u5 code line ;then
#############==========================================================================================
        my_ftpcmd="${t_cmd}"
        ...
        ####  JPT 2 0071107  handling shell commands
        ####
        echo "DEBUG LINE JPT 20071107 before: my_ftpcmd = --->${my_ftpcmd}<---"   >> ${batch_log}.debug
        if [ "X${my_ftpcmd}" = "X!" ]; then
          echo "DEBUG LINE JPT 20071107 during: my_ftpcmd = --->${my_ftpcmd}<---"   >> ${batch_log}.debug
          my_ftpcmd="shell"
        fi
        echo "DEBUG LINE JPT 20071107  after: my_ftpcmd = --->${my_ftpcmd}<---"   >> ${batch_log}.debug
        ...
        echo "DEBUG LINE JPT 20071107: this_e_string = ${this_e_string}"   >> ${batch_log}.debug
        echo "DEBUG LINE JPT 20071107: this_s_string = ${this_s_string}"   >> ${batch_log}.debug
        ...
        do
          ...
          if [ ${ftpS} -gt 0 ];then
            ...
            my_logfile_trailer="${my_retcod} ${my_retstr}"       ## JPT 20060220 
        ...
        ####  
        ####  for every "put" or "get" command, process the ftp q card...
        #### Added "get" logic - 20070727 RLC
        ####  
        if [[ ("${my_ftpcmd:-Undefined}" = "put") || ("${my_ftpcmd:-Undefined}" = "get") ]]; then
          my_sum_line="${my_ftpcmd} ${my_params} "       ##  JT 20070503
          echo "${my_sum_line} " >> ${my_ftp_summary_log}  ##  JT 20070503
          ...
          if [ "${this_qcard}" = "UNDEFINED" ]; then
            msg="ERROR: processing ${num_analyzed} ${my_ftpcmd} commands but there are only ${num_files_sending} ftp_q_cards "
            echo "    ${msg} " >> ${my_ftp_summary_log}  ##  JT 20070503
            echo "ABOUT to call batcherror_notify.  \n ${msg}" >> ${batch_log}.debug
            batcherror_notify "${msg}"     ##  cannot continue analysis after this error. exit program. 
          fi
          if [ ! -a ${this_qcard} ]; then
            msg="ERROR: : Cannot read FTP QUEUE CARD ${this_qcard}    Processing ${num_analyzed} ${my_ftpcmd} commands: "
            echo "    ${msg} " >> ${my_ftp_summary_log}  ##  JT 20070503
            echo "ABOUT to call batcherror_notify.  \n ${msg}" >> ${batch_log}.debug
            ...
            if [ "${cmd_successful:-Undefined}" = "yes" ]; then
              msg="SUCCESSFUL FTP ${my_ftpcmd} to ${ftp_dest}, ${my_ftpcmd} ${t_cmd_args} "
              echo "${msg}" >> ${batch_log}.debug
              echo "##- MSG FROM ftp_deliver.ksh -## ${msg}"  >> ${this_qcard}   ## contents of $msg used again for status_rpt
              flagfile_prefix="FTP_S."
              my_logfile_trailer="${my_retcod} ${my_retstr}"       ## JPT 20060220 
              if read -u5 my_retcod my_retstr ;then
                echo "##- MSG FROM ftp_deliver.ksh -## ${my_retcod} ${my_retstr}"  >> ${this_qcard}  # Handle the line reporting number of bytes sent. 
                echo "    SUCCESSFUL FTP ${my_retstr}" >> ${my_ftp_summary_log}  ##  JT 20070503
              else
                detected_error=1
                err_count=`expr ${err_count} + 1`
                errmsgs="${errmsgs}ERROR - ${err_count} - Unexpected end of FTP LOG file. "
                echo "    ERROR - ${err_count} - Unexpected end of FTP LOG file. " >> ${my_ftp_summary_log}  ##  JT 20070503
                found_response=yes
                cmd_successful="no"
                leaveloop=yes
              fi
              ...
              msg="FAILED FTP ${my_ftpcmd} to ${ftp_dest}, ${my_ftpcmd} ${t_cmd_args} "
              echo "    ${msg}" >> ${my_ftp_summary_log}  ##  JT 20070503
              echo "${msg}" >> ${batch_log}.debug
              echo "##- MSG FROM ftp_deliver.ksh -## ${msg}"  >> ${this_qcard}   ## contents of $msg used again for status_rpt
              msg="WILL RETRY FTP to ${ftp_dest}, ${my_ftpcmd} ${t_cmd_args} "
              echo "    ${msg}" >> ${my_ftp_summary_log}  ##  JT 20070503
              echo "${msg}" >> ${batch_log}.debug
              echo "##- MSG FROM ftp_deliver.ksh -## ${msg}"  >> ${this_qcard}   ## contents of $msg used again for status_rpt

              flagfile_prefix="FTP_E."
              ####  mv ${this_qcard} ${BATCH_FTP_QUEUE}/errors  # move to errors directory                  ####  JPT Removed 20070525
              mv ${this_qcard} ${BATCH_FTP_QUEUE}/ready  # move to ready directory to be attempated again   ####  JPT 20070525
              if [ ! -a ${BATCH_FTP_QUEUE}/ready/`basename ${this_qcard}` ]; then                           ####  JPT 20070525
              ####  if [ ! -a ${BATCH_FTP_QUEUE}/errors/`basename ${this_qcard}` ]; then                    ####  JPT Removed 20070525
                ####  messagelog_notify "ERROR in command:  mv ${this_qcard} ${BATCH_FTP_QUEUE}/errors"     ####  JPT Removed 20070525
                messagelog_notify "ERROR in command:  mv ${this_qcard} ${BATCH_FTP_QUEUE}/ready"            ####  JPT 20070525
              fi
            fi
        ...
        ####
        #### Evaluation and reporting of analaysis of "get" command.
        #### RLC - 20070727
        ####
        if [ "${my_ftpcmd:-Undefined}" = "get" ];then
          if read -u4 t_cmd t_cmd_args ;then
            echo ${t_cmd_args} >> ${ftp_status_rpt}
            eval ${t_cmd_args}
            if [ $? -ne 0 ];then #could not execute directory listing
              msg "ERROR - ftp-get file not found...${t_cmd_args}"
              echo "${msg}" >> ${ftp_status_rpt}
            fi
          fi 
          if read -u4 t_cmd t_cmd_args ;then
            echo ${t_cmd_args} >> ${ftp_status_rpt}
            eval ${t_cmd_args}
            if [ $? -ne 0 ];then #could not move file
              msg "ERROR - ftp-get file not able to be moved to destination directory..."
              echo "${msg}" >> ${ftp_status_rpt}
              msg "    Failed command - ${t_cmd_args} "
              echo "${msg}" >> ${ftp_status_rpt}
            else
              msg="SUCCESSFUL - get file moved: ${t_cmd_args}"
              echo "${msg}" >> ${ftp_status_rpt}
            fi
          fi             
        fi
        ####  
        ####  for every "dir" command, record evidence of file on remote system in my_ftp_summary_log  ##   JT 20070503
        ####  
        if [ "${my_ftpcmd:-Undefined}" = "dir" ]; then
          echo "    ${my_logfile_trailer} " >> ${my_ftp_summary_log}  ##  JT 20070503
        fi


        echo "BACK FROM EVALUATING COMMAND...."  >>  ${batch_log}.debug 
      else        ####  end of u5
        detected_error=1
        err_count=`expr ${err_count} + 1`
        errmsgs="${errmsgs}ERROR - ${err_count} - Unexpected end of FTP LOG file. "
        echo "${errmsgs}ERROR - ${err_count} - Unexpected end of FTP LOG file....."  >>  ${batch_log}.debug 
        leaveloop=yes
      fi
    else        ####  end of u4
      detected_error=1
      ...
      echo "${errmsgs}ERROR - ${err_count} - Unexpected end of FTP command file....."  >>  ${batch_log}.debug 
      leaveloop=yes
    fi
  ...
  echo "CLOSED LOG and COMMAND files  ....."  >>  ${batch_log}.debug 

  if [ ${err_count} -gt 0 ]; then
    messagelog_notify "${errmsgs}"
  fi

  echo "######  FTP ANALYSIS COMPLETE for `basename ${batch_log}` with PID $$ on `date` "  >> ${my_ftp_summary_log}  ##  JT 20070503
...
{
        ####
        #### Addition of Direction to allow get command
        #### RLC - 20070727
        ####
    unset this_mode this_dest_file this_source_file this_direction final_dest_file
    ...
    this_direction=`grep "^##- Direction *-## " ${ftp_queue_card} |head -1 |sed 's/##- Direction *-## //' `
...
####  
    ...
    ret_stat=${?}
###    if [ "${ret_stat}" != "0" ];then
    if [[ ("${ret_stat}" != "0") && ("${this_direction}" != "get") ]];then
      ...
      #### 
      #### Determining destination when doing a get command.
      #### Creation of commands specific to get a file to be evaluated later.
      #### RLC - 20070727
      ####
      if [ "${this_direction}" = "get" ];then
       final_dest_file=${this_dest_file}
       this_dest_file=${BATCH_FTP_GET:-Undefined}/${this_dest_file##*/}
       echo " ${this_direction} ${this_source_file} ${this_dest_file} " >> ${ftp_tmp_cmd_file}
       echo " !#GET_CMD ls -la ${final_dest_file} " >> ${ftp_tmp_cmd_file}
       echo " !#GET_CMD mv ${this_dest_file} ${final_dest_file} " >> ${ftp_tmp_cmd_file}
       dirstr=`echo " dir ${this_source_file}" |sed 's/(+1)//g'`
      else
       echo " ${this_direction} ${this_source_file} ${this_dest_file} " >> ${ftp_tmp_cmd_file}
...
setlog ${batch_prg}.${batch_start_dtm}    ## debug    write a new file each time this runs.
