...
#*******************************************************************************
#
#  NAME:              FPA_FIFOstart.ksh  (First In First Out)
#
...
#                     specific job has been successfully entered in the FIFO 
#                     queue and will be starting later.  This will provide the 
#                     user with feedback in the summary log AT the time that 
#                     the FPA tries to initiate the job.  Unless there is a 
...
if [ ${num_args} -lt 2 ]; then
  ...
  batchstart
  msg="ERROR -- USAGE: FPA_FIFOstart.ksh SYNCFILE <job with optional parameters>. ${*:-SyncfileNotSpecified} "
...
logfilename="FPA_FIFOstart.${batchsync_filename}.`date +'%Y%m%d'`.log"
...
if [ ${OK} -ne 0 ]; then
  msg="Problem executing ${cmd} ( `which ${cmd}` ) "
  batcherror_notify "${msg}"
fi
