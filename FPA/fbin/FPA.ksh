...
#                      ${BATCH_JOBCARDS}/FPA_IS_SUSPENDED.DAT - The existence 
#                          of this file will cause the FPA to start and 
#                          immediately finish without processing anything. A 
#                          line beginning with "#RELEASE_AFTER: YYYYMMDDHHMNS" 
#                          will cause the FPA to release itself after the 
#                          time specified in YYYYMMDDHHMNS. And a line 
#                          beginning with "#NOTIFY_AFTER: YYYYMMDDHHMNS" will 
#                          cause the FPA to begin sending ERROR emails each 
#                          time it finds itself suspended after YYYYMMDDHHMNS. 
#                          Additional comment lines describing the purpose for 
#                          suspending a specific FPA implementation are encouraged. 
#
#                      ${FPA_CONTROL_DIR}/FPA_IS_SUSPENDED.DAT - The existence 
#                          of this file will cause the FPA to start and 
#                          immediately finish without processing anything. A 
#                          line beginning with "#RELEASE_AFTER: YYYYMMDDHHMNS" 
#                          will cause the FPA to release itself after the 
...
#                          cause the FPA to begin sending ERROR emails each 
#                          time it finds itself suspended after YYYYMMDDHHMNS. 
#                          A line beginning with #MSG_TO_USERS:" will be 
#                          recorded in both the batch_summary.[date] and the 
#                          FPA.ksh.[date] log file in every FPA implementation 
#                          on this computer.
#                          Additional comment lines describing the purpose for 
#                          suspending all FPA implementations on this computer 
#                          are encouraged. 
#
...
#                      FPA_CONTROL_DIR - a directory where the 
#                          FPA_IS_SUSPENDED.DAT file must be created in order 
#                          to  suspend ALL FPA environment on this computer. 
#                      
...
# 04/18/2005   J Thiessen       Changed to reduce sync problems after shutdowns
# 04/12/2006   J Thiessen       Added batch day separator in summary log
# 07/13/2006   J Thiessen       Added SUSPEND functionality. If the file 
#                               ${BATCH_JOBCARDS}/FPA_IS_SUSPENDED.DAT exists, 
#                               then the FPA will not error and will not execute.  
# 08/19/2006   J Thiessen       Added SYSTEM SUSPEND functionality. If the 
#                               FPA_IS_SUSPENDED.DAT file exists in the 
#                               ${FPA_CONTROL_DIR:-/asp/FPA/FPA_CONTROL_DIR} 
#                               directory, then the FPA will exit without error
#                               and without processing files.  
# 04/14/2007   J Thiessen       Now will match commands only when the delimited 
#                               index string are the first characters on the 
#                               line in FPA_commands.dat
# 04/14/2007   J Thiessen       The ck env section now "eval"s ${job} and does 
#                               not notify if an environment is not ready, the 
#                               $job can notify if needed. 
#
#*******************************************************************************
...
####  
####  if first_run_of_day then record it and create tomorrow's summary log file
####  note that first_run_of_day is used again later...
####  
...
####
####  Check for new batch-day and make entry in summary log.  JPT 20060412
####
currdtm
batch_day_file="BatchDay.`echo ${batch_start_dtm:-00000000000000} |cut -c1-8`"
if [ ! -a ${BATCH_LOGS:-.}/${batch_day_file} -a `echo ${batch_start_dtm:-00000000000000} |cut -c9-12` -ge ${BATCH_START_OF_DAY:-0800} ]; then
  my_msg="----NEW_BATCH_DAY ${batch_prg:-NoProgram} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} | USER=${LOGNAME} | PID=${$} | MSG=Beginning ${batch_day_file} at `date +'%Y%m%d%H%M%S'` "
  echo "${my_msg}" >  ${BATCH_LOGS:-.}/${batch_day_file}
  echo "${my_msg}" >> ${BATCH_LOGS:-.}/batch_summary.`echo ${batch_start_dtm:-00000000000000} |cut -c1-8`
fi

####
####  Check whether or not ALL FPA implementations on this system have been suspended.  
####
releasing_fpa="FALSE"
unset release_msg
my_file=${FPA_CONTROL_DIR:-/asp/FPA/FPA_CONTROL_DIR}/FPA_IS_SUSPENDED.DAT
if [ -a ${my_file} ]; then
  echo "This file exists  ${my_file}"
  if [ ! -r ${my_file} ]; then    ## Check for additional information in the file if it is readable. 
    messagelog_summary "${batch_start_dtm} ALL FPA environments on this system have been SUSPENDED. A Perot Systems Support associate must remove the UNREADABLE file ${my_file} to resume normal FPA operations. "
  else
    ## 
    ## Check for more information since the file is readable.
    ## 
    #### message to users #############################################
    tmp=`grep "^#MSG_TO_USERS: " ${my_file} | tail -1 |cut -d" " -f2-`
    if [ ${#tmp} -gt 0 ]; then
      fpa_user_msg="MSG  ${tmp:-}  "  
    fi
    #### release time #################################################
    fpa_release_time=`grep "^#RELEASE_AFTER: " ${my_file} | tail -1 |cut -d" " -f2`
    tmp=`echo ${fpa_release_time:-} | grep [^0-9]`  # Confirm that this variable is numeric
    if [ ${#tmp} -gt 0 ]; then
      messagelog_summary "${batch_start_dtm} ERROR: INVALID #RELEASE_AFTER time ${tmp:-}"
      unset fpa_release_time
    fi
    #### notify time ##################################################
    fpa_notify_time=`grep "^#NOTIFY_AFTER: " ${my_file} | tail -1 |cut -d" " -f2`
    tmp=`echo ${fpa_notify_time:-} | grep [^0-9]`  # Confirm that this variable is numeric
    if [ ${#tmp} -gt 0 ]; then
      messagelog_summary "${batch_start_dtm} ERROR: INVALID #NOTIFY_AFTER time ${tmp:-}"
      unset fpa_notify_time
    fi
    ## 
    ## check the release time if it is available
    ## 
    if [ ${#fpa_release_time} -gt 0 ]; then
      release_msg="RESUMING at ${fpa_release_time} "
      if [ ${batch_start_dtm} -gt ${fpa_release_time} ]; then
        releasing_fpa="TRUE"
        messagelog_summary "${batch_start_dtm} REMOVING the FPA SUSPENSION FILE: ${my_file}"
        cat ${my_file} >> ${batch_log} 2>&1
        mv  ${my_file} `dirname $my_file`/FPA_IS_SUSPENDED.DAT.archive`date +"%Y%m%d%H%M%S%N"`
        if [ -a ${my_file} ]; then   ## Check to see if the mv command worked. NOTIFY if it failed. "
          batcherror_notify "ERROR -- UNABLE to lift the FPA's SUSPENSION. Cannot move the file: `ls -l ${my_file}` "
        else
          messagelog_summary "${batch_start_dtm} Lifting the SUSPENSION of ALL FPA environments on this system.  Normal FPA operations will resume on each environments next FPA cycle. "
        fi
      fi
    fi
    if [ "${releasing_fpa}" = "FALSE" ]; then
      if [ ${#fpa_notify_time} -gt 0 ]; then
        if [ ${batch_start_dtm} -gt ${fpa_notify_time} ]; then
          msg="${batch_start_dtm} WARNING: Extending SUSPENSION of ALL FPA environments past ${fpa_notify_time}. At ${batch_start_dtm} the file ${my_file} still exists. ${fpa_user_msg:-}"
          messagelog_notify  "${msg}"
          messagelog_summary "${msg}"
        else
          msg="${batch_start_dtm} NOTICE: ALL FPA environments are SUSPENDED by file: ${my_file}  ${fpa_user_msg:-}${release_msg:-}  Notify after ${fpa_notify_time} "
          messagelog_summary "${msg}"
        fi
      else
        msg="${batch_start_dtm} NOTICE: ALL FPA environments are SUSPENDED by file: ${my_file}  ${fpa_user_msg:-}${release_msg:-}  "
        messagelog_summary "${msg}"
      fi
    fi
  fi
  batchend       ##  EXIT the FPA without processing and without error because the FPA has been intentionally suspended.
fi

####
####  Check whether or not this specific FPA implementation is suspended.  
####
releasing_fpa="FALSE"
unset release_msg
my_file=${BATCH_JOBCARDS:-UndefinedVariable}/FPA_IS_SUSPENDED.DAT
if [ -a ${my_file} ]; then
  echo "Thsi file exists  ${my_file}"
  if [ ! -r ${my_file} ]; then    ## Check for additional information in the file if it is readable. 
    messagelog_summary "${batch_start_dtm} The FPA has been SUSPENDED. Remove the UNREADABLE file ${my_file} to resume normal FPA operations. "
  else
    ## 
    ## Check for more information since the file is readable.
    ## 
    ...
    if [ ${#fpa_release_time} -gt 0 ]; then
      ...
      if [ ${batch_start_dtm} -gt ${fpa_release_time} ]; then
        ...
        messagelog_summary "${batch_start_dtm} REMOVING the FPA SUSPENSION FILE: ${my_file}"
        cat ${my_file} >> ${batch_log} 2>&1
        rm  ${my_file} >> ${batch_log} 2>&1
        if [ -a ${my_file} ]; then   ## Check to see if the rm command worked. NOTIFY if it failed. "
          batcherror_notify "ERROR -- UNABLE to lift the FPA's SUSPENSION. Cannot remove the file: ${my_file} "
        else
          messagelog_summary "${batch_start_dtm} The FPA SUSPENSION is lifting. Normal operations will resume on the next start. "
        fi
      fi
    fi
    if [ "${releasing_fpa}" = "FALSE" ]; then
      if [ ${#fpa_notify_time} -gt 0 ]; then
        if [ ${batch_start_dtm} -gt ${fpa_notify_time} ]; then
          msg="${batch_start_dtm} WARNING: Extending FPA's SUSPENSION past ${fpa_notify_time}. At ${batch_start_dtm} the file ${my_file} still exists. ${fpa_user_msg:-}"
          messagelog_notify  "${msg}"
          messagelog_summary "${msg}"
        else
          msg="${batch_start_dtm} NOTICE: FPA is SUSPENDED by file ${my_file}  ${fpa_user_msg:-}${release_msg:-}  Notify after ${fpa_notify_time} "
          messagelog_summary "${msg}"
        fi
      else
        msg="${batch_start_dtm} NOTICE: FPA is SUSPENDED by file ${my_file}  ${fpa_user_msg:-}${release_msg:-}  "
        messagelog_summary "${msg}"
      fi
    fi
  fi
...
if [ ${#FPA_CHECK_ENV} -gt 0 ]; then
  if [ ! -r ${BATCH_ETC}/${FPA_CHECK_ENV} ]; then
    msg="ERROR -- Cannot read ${FPA_CHECK_ENV} to check this FPA environment"
  ...
  do
    ...
    if [ ${ret_stat} -eq 0 ]; then
      ...
      batcherror "ERROR -- FAILED TO EXECUTE environment check job: ${job}"   # EXIT WITH ERROR, The ENV check will notify if necessary.
    fi
  done
fi
...
####  
FPA_nohup.ksh FPA_processjobcards.ksh   #### JPT 20050418  Start this process in the background rather than foreground
if [ $? -ne 0 ]; then
  msg="WARNING -- PROBLEM executing FPA_processjobcards.ksh"
  messagelog_notify "${msg}"
fi
####  JPT 20060125   If FPA_processjbocards.uni exists, check to see if a warning or error is needed.  
if [ -r ${BATCH_SYSTEM}/sync_files/FPA_processjobcards.ksh.uni ]; then
  FPA_processjobcards_attempts=`wc -l ${BATCH_SYSTEM}/sync_files/FPA_processjobcards.ksh.uni |sed 's/  */ /g'|sed 's/^ //' |cut -d" " -f1 2>/dev/null`  
fi
if [ ${FPA_processjobcards_attempts:-0} -gt 20 ]; then    #### JPT 20050418  ERROR condition, send alarm
  msg="ERROR -- FPA_processjobcards.ksh -- probably need to remove FPA_processjobcards.ksh.uni in the sync_files directory. "
  messagelog_notify "${msg}"
elif  [ ${FPA_processjobcards_attempts:-0} -gt 2 ]; then    #### JPT 20050418  Warning condition, send notice
  msg="WARNING -- potential problem processing scheduled jobs.  Check FPA_processjobcards.ksh.uni in the sync_files directory. "
  messagelog_notify "${msg}"
fi
