...
####
#### Copy code to the code_waiting directory and move code to code_to_production_archive with seq number
####

copy_file ${inFileDir}/${inFileName} ${promFileDir}/${inFileName} 640 IGNORE

#error check the copy
ret_val=${?}
...
if [ ${ret_val} -ne 0 ];then
  ...
  msg="CODE_PROMOTION - File ${inFileName} was copied to the ${BATCH_SYSTEM}/code_waiting directory on `date +'%D'` at `date +'%R'`"
  echo "${msg}" >> ${BATCH_LOGS}/FPA.*.cron.log.`date +'%B'`
fi
