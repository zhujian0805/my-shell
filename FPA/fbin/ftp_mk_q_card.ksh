...
# 08/2007      R. Crawford      Logic for get of file.
# 11/07/2007   J Thiessen       Now able to specify additional commands, not 
#                               only 'site' commands. Use ^ to indicate a
#                               space in a command.  
#*******************************************************************************
...
####
ftp_dest=${1:-}
messagelog "Destination: ${ftp_dest}"
...
messagelog "ftp_ini_file - ${ftp_ini_file}"
...
if [ $# -gt 0 ]; then shift; fi   # shift away the argument that has been validated and stored. 
messagelog "Status var - ${ftp_status}"
messagelog "Delayed var - ${ftp_delayed}"
...
messagelog "Direction var - ${ftp_direction}"
messagelog "Mode var - ${ftp_mode}"
####
####  Validate transfer mode. 
####
##ftp_mode=`echo "${1:-}" | tr [A-Z] [a-z] `
case "${ftp_mode}" in
  (ascii)     : ;;
  (binary)    : ;;
...
if [ $# -gt 0 ]; then shift; fi   # shift away the argument that has been validated and stored. 
messagelog "Mode var after varification - ${ftp_mode}"

####
####  Validate source file. 
####
ftp_source_file="${1:-}"                    
messagelog "Source file at vailidation - ${ftp_source_file}"
if [[ ("${ftp_direction}" != "get") && ( ! -r ${ftp_source_file}) ]]; then
  msg="ERROR:  Cannot read source file '${ftp_source_file}' . " 
  batcherror "${msg} "
fi
messagelog "Direction within validate source - ${ftp_direction}"
if [ "${ftp_direction}" != "get" ];then
...
if [ $# -gt 0 ]; then shift; fi   # shift away the argument that has been validated and stored. 
messagelog "Source var after validation - ${ftp_source_file}"
...
ftp_dest_file="${1:-}"                    
if [ $# -gt 0 ]; then shift; fi   # shift away the argument that has been validated and stored. 
messagelog "Destination var - ${ftp_dest_file}"
...
if [ "${?}" -ne 0 ]; then batcherror_notify "ERROR - problem writing to ${tmp_ftp_q_card_name}"; fi
echo "##- File Size   -## ${ftp_file_size:-9999999999} " >> ${tmp_ftp_q_card_name}
if [ "${?}" -ne 0 ]; then batcherror_notify "ERROR - problem writing to ${tmp_ftp_q_card_name}"; fi

####
####  Process the additional commands, if any.  
####  20071107 JPT updated to handle commands more generically. Each command must be one 
####  word, use the '^' character to indicate a space in the command.
####
while [ ${#@} -gt 0 ]
do
  ...
  my_cmd_caret=`echo ${my_cmd:-} | grep "\^" |wc -l`
  if [ ${my_cmd_caret} -eq 0 ];then ####  20071107 JPT No space delimiters, insert site command to maintain compatability with earlier versions.  
    ...
    my_cmd_string=`echo ${my_cmd} | sed 's/\^/ /g'`
    echo "##- Command     -##   ${my_cmd_string}  " >> ${tmp_ftp_q_card_name}
  fi
  if [ "${?}" -ne 0 ]; then batcherror_notify "ERROR - problem writing to ${tmp_ftp_q_card_name}"; fi
