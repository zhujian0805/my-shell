...
# 03/21/2006   R. Crawford      Modified log copy from 666 permissions to 660
# 06/06/2006   J. Thiessen      Modified format of batch_summary messages    
#
#*******************************************************************************
...
if [ -a ${batchuniquefilename} ]; then 
  ...
  msg="NOTICE: Will not start ${$} ${batch_prg} at ${batch_start_dtm:-UnknownTime} due to conflicting job: ${conflict_msg:-UnknownJob}"
  messagelog_summary "${msg} "
  echo "${msg}" >> ${err_flag_name}
  echo "${msg}" >> ${batchuniquefilename}
...
logmsg_sum="--START ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} 
	 USER=${LOGNAME} | PID=$$ 
	 ARGS=${batch_args} 
	 THREAD=${batchthread_start_dtm:-NA} |"
if [ "${NO_SUMMARY_MSGS:-X}" != "TRUE" ];then
  echo "${logmsg_sum}" >> ${BATCH_LOGS}/batch_summary.`date +'%Y%m%d'`
fi
writelog "${logmsg}"
...
if [ "${NO_OUTPUT:-X}" = "X" ]; then
  ...
  make_dir "${batch_working_directory}" 2777 >> ${batch_log} 2>&1
  error_code=$?
...
currdtm
now_time_sec=`eval date +%s`
eTime=`elapsed_time ${batch_start_sec:-${now_time_sec}} ${now_time_sec}`
logmsg="--ERROR_NOTIFY ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} | USER=${LOGNAME} | PID=$$ | MSG=${1} | ELAPSED_TIME=${eTime:-Unknown} | "
logmsg_sum="--ERROR_NOTIFY ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} 
	 USER=${LOGNAME} | PID=$$ 
	 MSG=${1} 
	 ELAPSED_TIME=${eTime:-Unknown} | "
writelog "${logmsg}"
echo "${logmsg_sum}" >> ${BATCH_LOGS}/batch_summary.`date +'%Y%m%d'`
batch_notify "${logmsg}"
if [ "${batchsync_filename:-X}" != "X" ];then
...
now_time_sec=`eval date +%s`
eTime=`elapsed_time ${batch_start_sec:-${now_time_sec}} ${now_time_sec}`
logmsg="--ERROR ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} | USER=${LOGNAME} | PID=$$ | MSG=${1} | ELAPSED_TIME=${eTime:-Unknown} | "
logmsg_sum="--ERROR ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} 
	 USER=${LOGNAME} | PID=$$ 
	 MSG=${1} 
	 ELAPSED_TIME=${eTime:-Unknown} | "
echo "${logmsg_sum}" >> ${BATCH_LOGS}/batch_summary.`date +'%Y%m%d'`
...
now_time_sec=`eval date +%s`
eTime=`elapsed_time ${batch_start_sec:-${now_time_sec}} ${now_time_sec}`
logmsg="--END ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} | USER=${LOGNAME} | PID=$$ | MSG=Completed Successfully | ELAPSED_TIME=${eTime:-Unknown} | "
logmsg_sum="--END ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} 
	 USER=${LOGNAME} | PID=$$ 
...
currdtm
echo "----MSG from PID=${$} 
	 AT ${currdtm_dt} ${currdtm_tm} 
	 ${1:-Undefined_Message}" >> ${BATCH_LOGS}/batch_summary.`date +'%Y%m%d'`
...
#Replaced for Linux version
#mailx -s "d950 job: ${batch_prg}" ${mailtousers} << EOT
#"${1}"
#~.
#EOT
echo -e "${1}" | mail -s "Linux FPA job: ${batch_prg} - ${ORACLE_SID}" ${mailtousers}
return_status=$?
:
...
if [ return_status -ne 0 ]; then
  logmsg="--NOTIFICATION FAILURE ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} | USER=${LOGNAME} | PID=${$} | MSG=FAILED to complete notification process | "
  logmsg_sum="--NOTIFICATION FAILURE ${batch_prg} | ON ${batch_sys} | AT ${currdtm_dt} ${currdtm_tm} 
	 USER=${LOGNAME} | PID=${$} 
	 MSG=FAILED to complete nofication process | "
  echo "${logmsg_sum}" >> ${BATCH_LOGS}/batch_summary.`date +'%Y%m%d'`
else
...
if [ "X${0}" = "X-ksh" ]; then
  batch_prg="InteractiveProcessing"
else
